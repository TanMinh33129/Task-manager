  ...
  try {
    ...
    const usersWithStats = await Promise.all(
      users.map(async (user) => {
        const tasks = await Task.countDocuments({ user: user._id });
        const done  = await Task.countDocuments({ user: user._id, status: 'done' });
        return { ...user.toObject(), taskCount: tasks, doneCount: done };
      })
    );
